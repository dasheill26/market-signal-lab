...
11. **A stale-closure bug silently broke live price updates after the first symbol switch.** The WebSocket subscription effect had an empty dependency array, so the `price_update` handler inside captured the initially-selected symbol by closure and, since the effect never re-ran, compared every incoming update against that one frozen value forever. After a user switched symbols even once, every live update failed the check and got silently dropped — which is exactly why the chart stopped visibly moving. Fixed with a ref that's kept in sync with the current symbol on every render, read fresh inside the WebSocket callback instead of the stale closed-over variable.
12. **A companion resource leak in the same code path.** `unsubscribe` only called `leave_room()`, never actually removing the symbol from the backend's active-polling set — so every symbol a user ever visited in a session kept being polled every 30 seconds forever, even with zero remaining subscribers. Fixed with real per-symbol subscriber counting (a symbol leaves the active set only once its count reaches zero), plus a `disconnect` handler for the case a naive fix would still miss: a client closing the browser tab without ever calling `unsubscribe` explicitly.
13. **A live-data edge case the bundled fallback datasets don't happen to contain crashed model training with "Input X contains infinity."** A zero-volume day followed by a nonzero-volume day makes `volume_change`'s `pct_change()` produce a genuine mathematical infinity — percentage change from a zero base is undefined. The clean historical fallback data never exhibits this, so it only surfaced against live data, on gold specifically. Reproduced directly by injecting the exact trigger into a test rather than waiting for it to reappear by chance, and fixed at the single source every feature-consuming code path shares (`add_features()` itself), not patched into just the one call site that happened to crash first — a second, separate call site in the live-prediction path would have hit the identical bug otherwise.
...
34 tests: technical indicator correctness (RSI mathematically bounded 0–100, ATR always non-negative, no feature ever produces infinity even with an injected zero-volume day), the walk-forward fold-boundary guarantee, model comparison uses identical methodology for every candidate, hyperparameter tuning genuinely uses `TimeSeriesSplit` (checked directly, not just that it runs), the calibrated model's Brier score beats the 50/50 baseline, the cache returns identical results faster, API contract tests (every forecast response must include the disclaimer and data mode; the analysis endpoint must always return all four components together), the SPA routing regression test, the forex/gold/stock fallback regression tests (each asset class's fallback genuinely returns data in that asset class's own real price range), the gold ticker-mapping test, the WebSocket subscription-leak regression tests (unsubscribe, disconnect, and shared-symbol reference counting all checked directly against internal state), the position-size copy-paste bug regression test, and — the one that matters most for the risk panel — a direct check that its output never contains the words "buy," "sell," "long," or "short."
